    ...
    def __call__(self, image, target=None, mask=None):
        if self.min_size is None:
            ...
            if max_original_size / min_original_size * min_size > self.max_size:
                min_size = math.floor(min_original_size / max_original_size * self.max_size)

            resized_image = F.resize(img=image, size=min_size)
            # resized_image = F.resize(img=image, size=min_size, max_size=self.max_size)
