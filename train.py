...
def parse_args():
    ...
    parser.add_argument('-b', '--backbone', default='resnet50', type=str,
                                            help='resnet50, swin_tiny, swin_small, swin_large, swin_large_window12')
...
def train():
    ...
    print("batch size per GPU: " + str(cfg['batch_size']))
    print("base_lr: " + str(base_lr))
    print("backbone_lr: " + str(backbone_lr))
    print("----------------------------------------------------------")
        ...
        if (epoch + 1) % args.eval_epoch == 0 or (epoch + 1) == max_epoch:
            ...
            if distributed_utils.is_main_process():
                if evaluator is None:
                    ...
                    res = open('det_results.txt', mode='a')
                    res.write('epoch:' + str(epoch + 1) + " " + str(cur_map) + "\n")
                    res.close()
